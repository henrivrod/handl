...
let check (globals, functions) =
  ...
  let built_in_decls =
    let printInt = StringMap.add "print" {
      rtyp = PrimitiveType(Int);
      fname = "print";
      formals = [(PrimitiveType(Int), "x")];
      locals = []; body = [] } StringMap.empty in
    let printBool = StringMap.add "printb" {
      rtyp = PrimitiveType(Int);
      fname = "print";
      formals = [(PrimitiveType(Bool), "x")];
      locals = []; body = [] } printInt in
    let printFloat = StringMap.add "printf" {
      rtyp = PrimitiveType(Int);
      fname = "print";
      formals = [(PrimitiveType(Float), "x")];
      locals = []; body = [] } printBool in
    StringMap.add "prints" {
          rtyp = PrimitiveType(Int);
          fname = "print";
          formals = [(PrimitiveType(String), "x")];
          locals = []; body = [] } printFloat
  in
  ...
  let check_func func =
    ...
    let rec check_expr = function
      ...
      | Binop(e1, op, e2) as e ->
        ...
        let err = "illegal binary operator " ^ string_of_expr e
        in
